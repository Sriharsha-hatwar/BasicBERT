...
class InputBasicFeatures(object):
    """A single set of features of data."""
...
def convert_examples_to_two_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if target in basic.keys():
            if len(basic[target]['sam']) <5:
                for sam in basic[target]['sam']:
                    basic_sen, basic_idx = sam
                    b_sens.append(basic_sen)
                    b_ids.append(basic_idx)
            else:
                rand = random.sample(list(range(len(basic[target]['sam']))), 4)
                for ri in rand:
                    basic_sen, basic_idx = basic[target]['sam'][ri]
                    b_sens.append(basic_sen)
                    b_ids.append(basic_idx)
            b_mask = [0] * len(b_sens)
        else:
        ...
        basic_mask=[]
        basic_seg_ids=[]
        for _ in range(len(b_sens)):
            basic_sen = b_sens[_]
            basic_idx = b_ids[_]
            
            example.text_a = basic_sen
            example.text_b = basic_idx
        
            tokens_a = tokenizer.tokenize(basic_sen)  # tokenize the sentence
            tokens_b = None
            text_b = None
            try:
                text_b = int(example.text_b)  # index of target word
            ...
            #b_seg_mask = (segment_ids==1)
            
            basic_ids.append(input_ids)
            basic_mask.append(input_mask)
            basic_seg_ids.append(segment_ids)
            input_ids=None
            input_mask=None
            segment_ids=None
        if len(b_mask)==2:
            basic_ids = basic_ids*2
            basic_mask = basic_mask*2
            basic_seg_ids = basic_seg_ids*2
            b_mask = [0]*len(basic_ids)
        elif len(b_mask)==1:
            basic_ids = basic_ids*4
            basic_mask = basic_mask*4
            basic_seg_ids = basic_seg_ids*4
            b_mask = [0]*len(basic_ids)
        elif len(b_mask)==3:
            basic_ids += [basic_ids[1]]
            basic_mask += [basic_mask[1]]
            basic_seg_ids += [basic_seg_ids[1]]
            b_mask = [0]*len(basic_ids)
        ...
        features.append(
            InputBasicFeatures(
                ...
                _input_ids=basic_ids,
                _input_mask=basic_mask,
                _segment_ids=basic_seg_ids,
                b_mask=b_mask
            )
        )
...
def convert_examples_to_two_features_1basic(
    examples, label_list, max_seq_length, tokenizer, output_mode, args
):
    """Loads a data file into a list of `InputBatch`s."""
    label_map = {label: i for i, label in enumerate(label_list)}
