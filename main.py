...
def run_train(
    ...
    for epoch in trange(int(args.num_train_epoch), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            logger.info(f'segment_ids : {segment_ids.shape}')
            logger.info(f'label_ids : {label_ids.shape}')
            logger.info(f'input_ids_2 : {input_ids_2.shape}')
            logger.info(f'input_mask_2 : {input_mask_2.shape}')
            logger.info(f'segment_ids_2 : {segment_ids_2.shape}')
            logger.info(f'_input_ids : {_input_ids.shape}')
            logger.info(f'_input_mask : {_input_mask.shape}')
            logger.info(f'_segment_ids : {_segment_ids.shape}')
            '''
            # compute loss values
